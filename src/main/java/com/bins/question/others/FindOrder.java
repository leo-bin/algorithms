package com.bins.question.others;

import java.util.*;

/**
 * @author leo-bin
 * @date 2020/5/17 10:19
 * @apiNote 课程表 Ⅱ
 * 来源：leetcode-210
 * 链接：https://leetcode-cn.com/problems/course-schedule-ii/
 */
public class FindOrder {


    /**
     * 题目描述：
     * 1.现在你总共有 n 门课需要选，记为 0 到 n-1
     * 2.在选修某些课程之前需要一些先修课程
     * 3.例如，想要学习课程 0 ，你需要先完成课程 1 ，我们用一个匹配来表示他们: [0,1]
     * 4.给定课程总量以及它们的先决条件，返回你为了学完所有课程所安排的学习顺序
     * 5.可能会有多个正确的顺序，你只要返回一种就可以了
     * 6.如果不可能完成所有课程，返回一个空数组
     * <p>
     * 示例 1:
     * 输入: 2, [[1,0]]
     * 输出: [0,1]
     * 解释:
     * 1.总共有 2 门课程。要学习课程 1，你需要先完成课程 0
     * 2.因此，正确的课程顺序为 [0,1]
     * <p>
     * 示例 2:
     * 输入: 4, [[1,0],[2,0],[3,1],[3,2]]
     * 输出: [0,1,2,3] or [0,2,1,3]
     * 解释:
     * 1.总共有 4 门课程。要学习课程 3，你应该先完成课程 1 和课程 2
     * 2.并且课程 1 和课程 2 都应该排在课程 0 之后
     * 3.因此，一个正确的课程顺序是 [0,1,2,3]
     * 4.另一个正确的排序是 [0,2,1,3]
     * <p>
     * 说明:
     * 1.输入的先决条件是由边缘列表表示的图形，而不是邻接矩阵。详情请参见图的表示法
     * 2.你可以假定输入的先决条件中没有重复的边
     * <p>
     * 提示:
     * 1.这个问题相当于查找一个循环是否存在于有向图中。如果存在循环，则不存在拓扑排序，因此不可能选取所有课程进行学习
     * 2.通过 DFS 进行拓扑排序 - 一个关于Coursera的精彩视频教程（21分钟），介绍拓扑排序的基本概念
     * 3.拓扑排序也可以通过 BFS 完成
     *
     * @apiNote 思路：
     * 1.拓扑排序的思想
     * 2.基本上就是找入度为0的课程，然后将它存队列
     * 3.每一次只要将入度为0的课程作为最后的顺序
     * 4.时间复杂度：O(n)
     * 5.空间复杂度：O(n)
     */
    public static int[] findOrder(int numCourses, int[][] prerequisites) {
        //入度数组(index就是课程号，value就是入度数)
        int[] inDegree = new int[numCourses];
        //map来存所有课程的依赖关系，key=课程号，value=依赖key的课程号
        HashMap<Integer, List<Integer>> map = new HashMap<>(16);
        Queue<Integer> queue = new LinkedList<>();
        //拓扑排序结果
        List<Integer> result = new ArrayList<>();
        //1.初始化所有课程的依赖关系
        for (int i = 0; i < prerequisites.length; i++) {
            //2.初始化所有课程的入度数
            inDegree[prerequisites[i][0]]++;
            if (map.containsKey(prerequisites[i][1])) {
                map.get(prerequisites[i][1]).add(prerequisites[i][0]);
            } else {
                List<Integer> list = new ArrayList<>();
                list.add(prerequisites[i][0]);
                map.put(prerequisites[i][1], list);
            }
        }
        //2.将所有入度为0的课程入bfs队列，开始广度优先搜索
        for (int i = 0; i < inDegree.length; i++) {
            if (inDegree[i] == 0) {
                //课程号如队列
                queue.offer(i);
            }
        }
        //3.开始bfs搜索
        while (!queue.isEmpty()) {
            int course = queue.poll();
            result.add(course);
            if (map.containsKey(course) && map.get(course).size() != 0) {
                for (int sub : map.get(course)) {
                    //对应的课程的入度-1
                    inDegree[sub]--;
                    //将入度为0的入队列
                    if (inDegree[sub] == 0) {
                        queue.offer(sub);
                    }
                }
            }
        }
        return result.size() == numCourses ? result.stream().mapToInt(Integer::valueOf).toArray() : new int[0];
    }



    public static void main(String[] args) {

    }
}
